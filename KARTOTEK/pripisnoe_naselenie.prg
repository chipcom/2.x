...
Function view_reestr_pripisnoe_naselenie()
  ...
  Index On DToS( FIELD->dfile ) to ( cur_dir() + 'tmp_krtr' ) DESCENDING
  Go Top
  If Eof()
...
Function f3_view_r_pr_nas( oBrow )
  ...
  mywait()
  Select KRTF
  Index On FIELD->FNAME to ( cur_dir() + 'tmp_krtf' ) For FIELD->reestr == krtr->kod .and. Empty( FIELD->TIP_OUT )
  Go Top
  Do While !Eof()
  ...
  If !fl_csv .and. krtr->ANSWER == 1
    ...
    If fl
      ...
      Index On Str( FIELD->rees_zap, 6 ) to ( cur_dir() + 'tmp_krte' ) For FIELD->reestr == krtr->kod
      r_use( dir_server() + 'mo_kartp', dir_server() + 'mo_kartp', 'KARTP' )
      r_use( dir_server() + 'mo_krtp',, 'KRTP' )
      Index On Str( FIELD->rees_zap, 6 ) to ( cur_dir() + 'tmp_krtp' ) For FIELD->reestr == krtr->kod
      Go Top
      Do While !Eof()
...
Function preparation_for_pripisnoe_naselenie()
  ...
  Select TMP_KRTP
  Set Relation To FIELD->kod_k into KART
  Index On Str( FIELD->kod_k, 7 ) to ( cur_dir() + 'tmp_krtp' )
  Index On Upper( kart->fio ) + DToS( kart->date_r ) + Str( FIELD->kod_k, 7 ) to ( cur_dir() + 'tmp2krtp' )
...
Function edit_uchast_spisok()
  ...
  If !Empty( mfl_fio )
    ...
    len_fio := Len( mfl_fio )
  Endif
  Index On Str( FIELD->kod_k, 7 ) to ( cur_dir() + 'tmp_wq' )
  use_base( 'kartotek' )
  Go Top
  Do While !Eof()
...
Function spisok_pripisnoe_naselenie( par )
  ...
  If Empty( arr_mo )
    ...
    Index On FIELD->mo_pr to ( cur_dir() + 'tmp_kart2' ) For !kart->( Eof() ) .and. kart->kod > 0
    Go Top
    Do While !Eof()
