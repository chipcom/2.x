...
function mmpp_rab()
  ...
  return { ; // ИСПРАВЛЕННОЕ меню социального статуса (битовое)
    { 'Дошкольник', 1 }, ;  //
    { 'Организован', 2 }, ;  //
    { 'Неорганизован', 3 }, ;  //
    { 'Учащийся', 4 }, ;  //
...
// 16.02.26
function mmpp_kategor()
  
  return { ;  // ИСПРАВЛЕННОЕ меню категорий
    { 'Инвалид ВОВ', 1, 1 }, ;
    { 'Участник ВОВ', 2, 2 }, ;
...
// 16.02.26
function stm_kategor()
  
  return { ;  // код категории льготы
    { 'инвалиды войны', 1, 1 }, ;
...
function mm_SOC()
  ...
  local aSOC := { ;
    ...
    { 'инвалид I группы', 810 }, ;
    { 'инвалид I группы, участник СВО', 813 }, ;
    { 'инвалид I группы, член семьи участника СВО', 816 } ;
  }
...
Function retfamimot( ltip, fl_no, is_open_kfio )
  ...
  If kart->MEST_INOG == 9 // т.е. отдельно занесены Ф.И.О.
    ...
    Endif
    kfio->( dbSeek(  Str( kart->kod, 7 ) ) ) //find ( Str( kart->kod, 7 ) )
    If Found()
...
//  Return PadR( AllTrim( mfio ) + ' ' + iif( mpol == 'М', '(М)', '(Ж)' ), 49 )
  Return PadR( AllTrim( mfio ) + ' ' + iif( mpol == 'М', '(М)', '(Ж)' ), 35 )
...
Function correct_polis_from_sptk()
  ...
  Index On Str( FIELD->kod_k, 7 ) + Str( Descend( FIELD->k_data ), 10 ) to ( cur_dir() + 'tmp_human' ) For FIELD->k_data > mdate
  ...
  Do While kart->kod > 0 .and. !Eof()
    ...
    Do While human->kod_k == kart->kod .and. !Eof()
      If !Empty( FIELD->k_data ) .and. FIELD->kod > 0 .and. ;
          Between( human_->VPOLIS, 1, 3 ) .and. FIELD->schet > 0 .and. ; // в счете
