...
//
// 06.02.23 проверка условия для применения КСЛП=3 для 2024 года
function conditionKSLP_3_24(aKSLP, DOB, n_date, profil, lshifr, lpar_org, arr_diag, duration)
...
// 16.03.23 проверка условия для применения КСЛП=19 для 2024 года
function conditionKSLP_19_24(aKSLP, DOB, n_date, profil, lshifr, lpar_org, arr_diag, duration)
  local fl := .f.
...
// 28.04.23 проверка условия для применения КСЛП=21 для 2024 года
function conditionKSLP_21_24(aKSLP, DOB, n_date, profil, lshifr, lpar_org, arr_diag, duration)
  local fl := .f.
...
// 28.04.23 проверка условия для применения КСЛП=22 для 2024 года
function conditionKSLP_22_24(aKSLP, DOB, n_date, profil, lshifr, lpar_org, arr_diag, duration)
...
// 28.04.23 проверка условия для применения КСЛП=23 для 2024 года
function conditionKSLP_23_24(aKSLP, DOB, n_date, profil, lshifr, lpar_org, arr_diag, duration)
  local fl := .f.
...
// 28.04.23 проверка условия для применения КСЛП=24 для 2024 года
function conditionKSLP_24_24(aKSLP, DOB, n_date, profil, lshifr, lpar_org, arr_diag, duration)
  local fl := .f.
...
// 28.04.23 проверка условия для применения КСЛП=25 для 2024 года
function conditionKSLP_25_24(aKSLP, DOB, n_date, profil, lshifr, lpar_org, arr_diag, duration)
  local fl := .f.
...
// 28.04.23 проверка условия для применения КСЛП=26 для 2024 года
function conditionKSLP_26_24(aKSLP, DOB, n_date, profil, lshifr, lpar_org, arr_diag, duration)
  local fl := .f.
...
// 28.04.23 проверка условия для применения КСЛП=27 для 2024 года
function conditionKSLP_27_24(aKSLP, DOB, n_date, profil, lshifr, lpar_org, arr_diag, duration)
  local fl := .f.
...
// 28.04.23 проверка условия для применения КСЛП=28 для 2024 года
function conditionKSLP_28_24(aKSLP, DOB, n_date, profil, lshifr, lpar_org, arr_diag, duration)
  local fl := .f.
  local valid_KSG := { ;
  ...
  // КСЛП=28 проведение сопроводительной лекарственной терапии
  // при злокачественных новообразованиях у взрослых в условиях
  // дневного стационара в соответствии с клиническими рекомендациями
...
// 28.04.23 проверка условия для применения КСЛП=29 для 2024 года
function conditionKSLP_29_24(aKSLP, DOB, n_date, profil, lshifr, lpar_org, arr_diag, duration)
  local fl := .f.
...
// 13.02.24 проверка услувия для применения КСЛП=30 для 2024 года
function conditionKSLP_30_24(aKSLP, DOB, n_date, profil, lshifr, lpar_org, arr_diag, duration)
  // КСЛП=30 спальное место законному представителю, НУЖЕН ЗАПРОС
  // 3.1.1. Предоставление спального места и питания законному представителю несовершеннолетних
  // (детей до 4 лет, детей старше 4 лет при наличии медицинских показаний, детей-инвалидов
  // в возрасте до 18 лет) (КСЛП30, КСЛП31).
  ...
  // документации. Предоставление спального места и питания законному представителю
  // детей-инвалидов в возрасте до 18 лет осуществляется при предьявлении справки,
  // подтверждающей факт установления инвалидности по форме, утвержденной приказом
  // Минздравсоцразвития России от 24.11.2010 № 1031н  "О формах справки, подтверждающей
...
// 13.02.24 проверка услувия для применения КСЛП=31 для 2024 года
function conditionKSLP_31_24(aKSLP, DOB, n_date, profil, lshifr, lpar_org, arr_diag, duration)
  // КСЛП=31 спальное место законному представителю, НУЖЕН ЗАПРОС
  // 3.1.1. Предоставление спального места и питания законному представителю несовершеннолетних
  // (детей до 4 лет, детей старше 4 лет при наличии медицинских показаний, детей-инвалидов
  // в возрасте до 18 лет) (КСЛП30, КСЛП31).
  // Предоставление спального места и питания законному представителю, при возрасте
...
// 13.02.24 проверка условия для применения КСЛП=32 для 2024 года
function conditionKSLP_32_24(aKSLP, DOB, n_date, profil, lshifr, lpar_org, arr_diag, duration)
  local fl := .f.
  local valid_KSG := { ;
...
// 13.02.24 проверка условия для применения КСЛП=33 для 2024 года
function conditionKSLP_33_24(aKSLP, DOB, n_date, profil, lshifr, lpar_org, arr_diag, duration)
  local fl := .f.
  local valid_KSG := { ;
    'st19.084', 'st19.085', 'st19.086', 'st19.087', 'st19.088', 'st19.089', ;
    'st19.094', 'st19.095', 'st19.096', 'st19.097', 'st19.098', 'st19.099', 'st19.100', 'st19.101', 'st19.102', ;
...
// 13.02.24 проверка условия для применения КСЛП=34 для 2024 года
function conditionKSLP_34_24(aKSLP, DOB, n_date, profil, lshifr, lpar_org, arr_diag, duration)
  local fl := .f.
  local valid_KSG := { ;
    'st19.084', 'st19.085', 'st19.086', 'st19.087', 'st19.088', 'st19.089', ;
    'st19.094', 'st19.095', 'st19.096', 'st19.097', 'st19.098', 'st19.099', 'st19.100', 'st19.101', 'st19.102', ;
...
// 13.02.24 проверка условия для применения КСЛП=35 для 2024 года
function conditionKSLP_35_24(aKSLP, DOB, n_date, profil, lshifr, lpar_org, arr_diag, duration)
  local fl := .f.
  local valid_KSG := { ;
    'ds19.058', 'ds19.059', 'ds19.060', 'ds19.061', 'ds19.062', ;
...
// 13.02.24 проверка условия для применения КСЛП=36 для 2024 года
function conditionKSLP_36_24(aKSLP, DOB, n_date, profil, lshifr, lpar_org, arr_diag, duration)
  local fl := .f.
  local valid_KSG := { ;
    'ds19.058', 'ds19.059', 'ds19.060', 'ds19.061', 'ds19.062', ;
    'ds19.067', 'ds19.068', 'ds19.069', 'ds19.070', 'ds19.071', ;
...
// 13.02.24 проверка условия для применения КСЛП=37 для 2024 года
function conditionKSLP_37_24(aKSLP, DOB, n_date, profil, lshifr, lpar_org, arr_diag, duration)
  local fl := .f.
  local valid_KSG := { ;
    'ds19.058', 'ds19.059', 'ds19.060', 'ds19.061', 'ds19.062', ;
    'ds19.067', 'ds19.068', 'ds19.069', 'ds19.070', 'ds19.071', ;
