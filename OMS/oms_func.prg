...
// 27.05.26 функция проверки лицензии на диспансеризацию/профилактику
Function license_for_dispans( _tip, _n_data, _ta )
  ...
  Static arr_date_disp := { ;
    ...
    { 131940, 1, 0, 20260101 } ;   // 131940;ФГБУЗ ФМБА России;+;;01.01.2026
   }
  ...
  if ( i := ascan(arr_date_disp, { | x | x[ 1 ] == glob_mo()[ _MO_KOD_TFOMS ] } ) ) > 0
    ...
    elseif arr_date_disp[ i, 4 ] > _n_data
      aadd( _ta, 'У МО лицензия на ' + mm_tip[ _tip ] + ' с ' + date_8( arr_date_disp[ i, 4 ] ) + 'г.' )
    endif
  else
    aadd( _ta, 'У МО нет лицензии на ' + mm_tip[ _tip ] )
  endif
