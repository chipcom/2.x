...
Function oms_double( k )
  ...
  do case
    case k == 0
      mas_pmt := { ;
        '~Собрать двойной случай', ;
        'Список ~двойных случаев', ;
        '~Разделить двойной случай' ;
      }
      mas_msg := { ;
        'Добавление двойного случая (составить из двух случаев один двойной)', ;
          ...
          if i == 0
            ...
            if Alpha_Browse( T_ROW, 2, maxrow() - 2, 77, 'f1ret_oms_human', color0, ;
              ...
              if ( glob_perso2 := tmp_h->kod ) == 0
                func_error( 4, 'Не найдено нужных записей!' )
              elseif ! ( ldiag == human->kod_diag ) .and. ! ( diagnosis_for_replacement( ldiag, human_->USL_OK, .t. ) .or. diagnosis_for_replacement( human->kod_diag, human_->USL_OK, .t. ) )
            ...
            if rslt_fl1 .or. rslt_fl2
              if rslt_sl1 != rslt_sl2
                n_message( { ;
                            'Результаты лечения в случаях отличаются:', ;
            ...
            if mkod > 0
              ...
              if G_SLock( str_sem2 )
                ...
                if f_Esc_Enter( 'создания двойного случая', .t. )
                  ...
                  recount_double_sl( glob_perso, lk_data2 ) // считаем по окончанию случая
...
Function input_double_sl( par )
  ...
  Static srec := 0
  Local arr_m, buf24, blk, t_arr[BR_LEN], ret
...
function exist_reserve_KSG(kod_pers, aliasHUMAN)
  ...
  If glob_mo[ _MO_KOD_TFOMS ] != '805965' // РДЛ
    ...
    set relation to FIELD->u_kod into __USL
    find ( str( kod_pers, 7 ) )
