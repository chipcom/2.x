...
Function inf_dvn( k )
  ...
  Case k == 1
    ...
    mas_msg := { ;
      'Распечатка карты учёта диспансеризации (профилактических мед.осмотров) №131/у', ;
  ...
  Case k == 14
    mas_pmt := { ;
      '~Сведения о диспансеризации по состоянию на ...', ;
      '~Индикаторы мониторинга диспансеризации взрослых' }
    mas_msg := { ;
      'Приложение к Приказу МЗВО №2066 от 01.08.2013г.', ;
      'Индикаторы мониторинга диспансеризации взрослых' }
    mas_fun := { ;
      'inf_DVN(21)', ;
      'inf_DVN(22)' }
  ...
  Case k == 16
    mas_pmt := { ;
      ...
      '~Файлы обмена (R11)' }
    mas_msg := { ;
      'Создание и просмотр файлов обмена R05...', ;
  ...
  Case k == 41
    ...
    mas_msg := { ;
      'Создание файла обмена R05... с планом-графиком по месяцам', ;
  ...
  Case k == 42
    ...
    // if glob_mo()[_MO_KOD_TFOMS] == '711001' // ЖД-больница
    mas_pmt := { ;
      '~Создание файлов обмена', ;
      '~Просмотр файлов обмена' }
    mas_msg := { ;
      'Создание файлов обмена R01... по всем месяцам', ;
...
//    If glob_mo()[ _MO_KOD_TFOMS ] == '711001' // ЖД-больница
      mas_pmt := { ;
        ...
        '~Просмотр файлов обмена' }
      mas_msg := { ;
        'Создание файлов обмена R11... за конкретный месяц', ;
...
*/
  ...
  If ( st_a_uch := inputn_uch( T_ROW, T_COL -5,,, @lcount_uch ) ) != NIL ;
    ...
    mywait()
    dbCreate( cur_dir() + 'tmp', { ;
      { 'gruppa_1', 'N', 1, 0 }, ;// 1-группа 2- группа 3 - без группа
...
//      For i := 1 To Max( count_dvn_arr_umolch18, count_dvn_arr_umolch )
      For i := 1 To Max( Len( dvn_arr_umolch( 0d20180101 ) ), count_dvn_arr_umolch )
...
Function f2_inf_dvn_svod( par, kod_h ) // сводная информация
  ...
  larr_dvn := iif( is_2018, dvn_arr_usl( 0d20180101 ), dvn_arr_usl )    // dvn_arr_usl18(), dvn_arr_usl )
  lcount_u := iif( is_2018, count_dvn_arr_umolch18, count_dvn_arr_umolch )
  larr_dvn_u := iif( is_2018, dvn_arr_umolch( 0d20180101 ), dvn_arr_umolch )    //dvn_arr_umolch18(), dvn_arr_umolch )
      ...
      If metap != 2
        If is_2018
          If lshifr == '2.3.3' .and. hu_->PROFIL == 3 ; // акушерскому делу
            .and. ( i := AScan( dvn_arr_usl( 0d20180101 ), {| x| ValType( x[ 2 ] ) == 'C' .and. x[ 2 ] == '4.20.1' } ) ) > 0
  ...
  For i := 1 To lcount
    If !Empty( larr[ 1, i ] )
      ...
      If metap != 2
        If is_2018
          If hu_->PROFIL == 3 .and. ;
              AScan( dvn_arr_usl( 0d20180101 ), {| x| ValType( x[ 2 ] ) == 'C' .and. x[ 2 ] == '4.20.1' } ) > 0
...
Function f21_inf_dvn_svod18( par )
  ...
  If par == 1
    For i := 1 To Len( dvn_arr_usl( 0d20180101 ) )    //  count_dvn_arr_usl18
...
//          AAdd( arr, { dvn_arr_usl18()[ i, 1 ], &mvard, '', i, &mvaro } )
    ...
    Next
  Else
    For i := 1 To Len( dvn_arr_umolch( 0d20180101 ) )   //  count_dvn_arr_umolch18
...
//        AAdd( arr, { dvn_arr_umolch18()[ i, 1 ], iif( dvn_arr_umolch18()[ i, 8 ] == 0, mn_data, mk_data ), '', i, 0 } )
        AAdd( arr, { dvn_arr_umolch( 0d20180101 )[ i, 1 ], iif( dvn_arr_umolch( 0d20180101 )[ i, 8 ] == 0, mn_data, mk_data ), '', i, 0 } )
      Endif
    Next
  Endif
