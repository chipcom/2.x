...
//  SMONTH := MONTH_UPLOAD // МЕСЯЦ
  ...
  Do While !Eof()
    ...
    If rees->kol_err < 0
      fl := func_error( 4, 'В файле PR11 за ' + lstr( rees->NMONTH ) + '-й месяц ' + ;
        lstr( YEAR_UPLOAD_DISPANSER ) + 'г. ошибки на уровне файла! Операция запрещена' )
    Elseif Empty( rees->answer )
      fl := func_error( 4, 'Файл PR11 за ' + lstr( rees->NMONTH ) + '-й месяц ' + ;
        lstr( YEAR_UPLOAD_DISPANSER ) + ' года не был прочитан! Операция запрещена' )
    Else
      Select MO_XML
      find ( Str( rees->kod, 6 ) )
      If Found()
  ...
  If fl_1 // .or. code_lpu == '321001'// не первый раз
    ...
    If fl
      ...
      Index On Str( FIELD->reestr, 6 ) + Str( FIELD->kod_k, 7 ) to ( cur_dir() + 'tmp_dr01k' )
      ...
      Index On Str( FIELD->reestr, 6 ) to ( cur_dir() + 'tmp_dr00' )
    ...
    If fl
      ...
      For v := 1 To 5
        ...
        Do While skol[ j ] > 0
          Select TMP
          If j == 2
            Index On FIELD->kod to ( cur_dir() + 'tmp_dr00' ) For tmp->tip == 2 .and. tmp->n_q == 0 // DESCENDING
          Elseif j == 1
            Index On FIELD->kod to ( cur_dir() + 'tmp_dr00' ) For tmp->tip == 1 .and. tmp->tip1 == 0 .and. tmp->n_q == 0 // DESCENDING
          Else
          ...
          Select TMP
          If j == 2
            Index On FIELD->kod to ( cur_dir() + 'tmp_dr00' ) For tmp->tip == 2 .and. tmp->n_m > 0
          Elseif j == 1
            Index On FIELD->kod to ( cur_dir() + 'tmp_dr00' ) For tmp->tip == 1 .and. tmp->tip1 == 0 .and. tmp->n_m > 0
          Else
...
Function f1_create_r11( lm, fl_dr00 )
  ...
  If fl_dr00
    ...
    Index On Str( FIELD->kod, 7 ) to ( cur_dir() + 'tmp_dr00' )
  Endif
  Use ( cur_dir() + 'tmp_00' ) New Alias TMP
...
Function delete_reestr_r11()
  ...
  Index On Descend( DToS( FIELD->DWORK ) + FIELD->TWORK1 ) to ( cur_dir() + 'tmp_dr01m' )
  Go Top
  If Eof()
...
Function delete_month_r11()
  ...
  Endif
  g_use( dir_server() + 'mo_xml',, 'MO_XML' )
  Index On Str( FIELD->reestr, 6 ) to ( cur_dir() + 'tmp_xml' ) For FIELD->tip_in == _XML_FILE_R12 .and. FIELD->TIP_OUT == 0
  g_use( dir_server() + 'mo_dr01',, 'REES' )
  g_use( dir_server() + 'mo_dr01m',, 'R01m' )
  Go Top
  Do While !Eof()
