...
function array_task_DB( array_DB )
  ...
  if HB_ISNIL( array_task )
    ...
    // листы учёта ОМС
    array_task[X_OMS, 1] := len(array_DB)
    // реестры и счета
    array_task[X_OMS, 2] := len(array_DB)
    // приёмный покой
    array_task[X_PPOKOJ, 1] := len(array_DB)
    array_task[X_PPOKOJ, 2] := len(array_DB)
    // платные услуги
    array_task[X_PLATN, 1] := len(array_DB)
    array_task[X_PLATN, 2] := len(array_DB)
    // ортопедия
    array_task[X_ORTO, 1] := len(array_DB)
    array_task[X_ORTO, 2] := len(array_DB)
    // касса ЛПУ
    array_task[X_KASSA, 1] := len(array_DB)
    array_task[X_KASSA, 2] := len(array_DB)
    // КЭК
//    array_task[X_KEK, 1] := len(array_DB)
//    array_task[X_KEK, 2] := len(array_DB)
    // госпитализация
    array_task[X_263, 1] := len(array_DB)
    array_task[X_263, 2] := len(array_DB)
  endif
  return array_task
