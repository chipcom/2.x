...
*/
  ...
  Local kartotek := { ;
    ...
    { 'PC3',        'C',  10, 0 }, ; // Социальная категория: '000' - нет, '035' - участник специальной военной операции (СВО), уволенный в запас (отставку), '065' - член семьи участника СВО, '810' - инвалид I группы, '813' - инвалид I группы, участник СВО, '816' - инвалид I группы, член семьи участника СВО
  ...
  Local human := { ;
    ...
    { 'MO_PR',      'C',   6, 0 }  ; // Код МО, выбранной застрахованным лицом для получения первичной медикосанитарной помощи по справочнику F032 (код ФФОМС)
  }
  //
  Local human_ := { ;
  ...
  Local human_2 := { ;
    ...
    { 'PN2',        'N',  10, 0 }, ; // для абортов, болевых синдромов
...
//  { 'SIZ_INJ',    'N',  15, 2 }, ;  // Стоимость израсходованного лек. препарата
  ...
  //
  Local human_u := { ;
    { 'KOD',        'N',  7, 0 }, ; // код пациента по файлу 'human'
    ...
    { 'KOL_RCP',    'N',  2, 0 }, ;
    { 'USL_REPL',   'N',  1, 0 }  ; // флаг последующей замены услуги на нулевую, для диспансеризаций: 0 - не меням, 1 - меняем
  }
  //
  Local human_u_ := { ;
  ...
  Local mo_rees := { ; // Реестр сведений об оказанной медицинской помощи, направляемый в ТФОМС 'mo_rees'
    ...
    { 'RES_TFOMS',  'N', 1, 0 }  ; // результат проверки реестра сета в ТФОМС ( 1-счет принят, 2-ошибка всего реестра, 3-ошибка в записях реестра, 4-процесс чтения)
  }
...
// 10.12.25 инициализация всех файлов инф.сопровождения по диспансерному наблюдению
