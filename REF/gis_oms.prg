...
Function f1edit_lic_addr( oBrow )
  ...
  oColumn := TBColumnNew( 'Адрес', {|| tmp_f038->ADDR } )
//  oColumn:colorBlock := blk
  oBrow:addcolumn( oColumn )
  status_key( '^<Esc>^ выход ^<Enter>^ просмотр' )
  Return Nil
...
Function f2edit_lic_addr( nKey, oBrow )
  ...
  Case nKey == K_ENTER
    ...
    f034->( dbSeek( tmp_f038->UIDSPMO + str( tmp_f038->IDADDRESS, 19 ) ) )
    do while f034->IDADDRESS == tmp_f038->IDADDRESS .and. f034->UIDSPMO == tmp_f038->UIDSPMO .and. ! f034->( Eof() )
      tmp_f034->( dbAppend() )
