...
function get_implantant()
  ...
  if len(_arr) == 0
    ...
    //   if (dbAlias)->TYPE == 'L'
    //     aadd(_arr, {str((dbAlias)->RZN, 6) + ' ' + alltrim((dbAlias)->NAME), ;
    //       (dbAlias)->RZN, dBegin, dEnd, ;
    //       (dbAlias)->ID, (dbAlias)->PARENT, alltrim((dbAlias)->LOCAL), ;
    //       alltrim((dbAlias)->MATERIAL), (dbAlias)->METAL, (dbAlias)->ORDER })
    //   endif
    //   (dbAlias)->(dbSkip())
    // enddo
