...
function getServiceForVMP(hVid, hMethod, sDiag)
  // hVid - вид ВМП (строка)
  ...
  for each row in arrVMP_USL
    ...
    if row[2] == vid .and. row[3] == hMethod .and. (ascan(arr, diag) > 0)
      ret := row[1]
      exit
    endif
  next
...
function getVMP_USL( dateSl)
  ...
  if len(arrVMP_USL) == 0
    ...
    //  1 - SHIFR(C)  2 - HVID(C)  3 - HMETHOD(N)   4 - DIAGNOZIS(C)
    (dbAlias)->(dbGoTop())
    do while !(dbAlias)->(EOF())
      aadd(arrVMP_USL, { alltrim((dbAlias)->SHIFR), alltrim((dbAlias)->HVID), (dbAlias)->HMETHOD, alltrim((dbAlias)->DIAGNOZIS) })
      (dbAlias)->(dbSkip())
    enddo
