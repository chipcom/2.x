...
//  Set( _SET_DATEFORMAT, 'yyyy-mm-dd' )
  db := opensql_db()
  aTable := sqlite3_get_table( db, 'SELECT tf_okato, orgtype, orgcod, nam_orgp, nam_orgk, tf_kod, smocod FROM f019 where smocod==' + code )
//      'datebeg, ' + ;
//      'dateend ' + ;
//      'FROM f019' )
  If Len( aTable ) > 1
...
Function get_SMO_OKATO_f019( code )
  ...
  If timeout_load( @time_load )
    ...
    Set( _SET_DATEFORMAT, 'yyyy-mm-dd' )
    db := opensql_db()
    aTable := sqlite3_get_table( db, 'SELECT tf_okato, orgtype, orgcod, nam_orgp, nam_orgk, tf_kod, smocod FROM f019 where orgtype=="2" and tf_okato==' + code )
//      'datebeg, ' + ;
//      'dateend ' + ;
//      'FROM f019' )
    If Len( aTable ) > 1
