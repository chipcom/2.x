...
function kslp(k,r,c)
    ...
    aadd(t_mas, '   ' + ' 1-связанная с возрастом (лица 75 лет и старше) (в том числе, включая консультацию врача-гериатра)')
    aadd(t_mas, '   ' + ' 3-Предоставление спального места и питания законному представителю (дети до 4 лет, дети старше 4 лет при наличии медицинских показаний)')
    aadd(t_mas, '   ' + ' 4-Проведение первой иммунизации против респираторно-синцитиальной вирусной инфекции в период госпитализации по поводу лечения нарушений, возникающих в перинатальном периоде, являющихся показанием к иммунизации')
    aadd(t_mas, '   ' + ' 5-Развертывание индивидуального поста')
    aadd(t_mas, '   ' + ' 6-Проведение сочетанных хирургических вмешательств')
    aadd(t_mas, '   ' + ' 7-Проведение однотипных операций на парных органах')
    aadd(t_mas, '   ' + ' 8-Проведение антимикробной терапии инфекций, вызванных полирезистентными микроорганизмами')
    aadd(t_mas, '   ' + ' 9-Наличие у пациента тяжелой сопутствующей патологии, осложнений заболеваний, сопутствующих заболеваний, влияющих на сложность лечения пациента (перечень указанных заболеваний и состояний')
    aadd(t_mas, '   ' + '10-Сверхдлительные сроки госпитализации, обусловленные медицинскими показаниями')
    ...
    oBox:View()
    if (ret := popup(5,19,15,62,t_mas,i,color0,.t.,"fmenu_reader",,;
        "Отметьте нужные КСЛП",col_tit_popup)) > 0
        m1var += '21:'
        for i := 1 to mlen
