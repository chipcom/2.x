...
function is_patient_invalid_for_reestr( human_kod, p_tip_reestr )
  // human_kod - код по базе HUMAN
  // p_tip_reestr - тип формируемого реестра

  local fl_DISABILITY := .f.

  If p_tip_reestr == 1
//    hb_main_curOrg // текущая организация    
...
function arr_fusl( human_kod )
  ...
  Select MOHU
  find ( Str( human_kod, 7 ) )
  Do While mohu->kod == human_kod .and. !Eof()
    AAdd( a_fusl, mohu->( RecNo() ) )
    Skip
  Enddo
  return a_fusl

function onko_lechenie( human_kod )  // Сведения о проведённых лечениях
  // human_kod - код по базе HUMAN
  ...
  If iif( human_2->VMP == 1, .t., Between( onksl->DS1_T, 0, 2 ) )
    Select ONKUS
    find ( Str( human_kod, 7 ) )
    Do While onkus->kod == human_kod .and. !Eof()
...
function onko_protivopokaz( human_kod )  // Сведения об имеющихся противопоказаниях
  // human_kod - код по базе HUMAN
  ...
  If human_->USL_OK < 3 // противопоказания по лечению только в стационаре и дневном стационаре
    Select ONKPR
    find ( Str( human_kod, 7 ) )
    Do While onkpr->kod == human_kod .and. !Eof()
...
function onko_diag( human_kod ) // Диагностический блок
  // human_kod - код по базе HUMAN
  ...
  If eq_any( onksl->b_diag, 98, 99 )
    Select ONKDI
    find ( Str( human_kod, 7 ) )
    Do While onkdi->kod == human_kod .and. !Eof()
...
function onko_napravleniya( human_kod ) // онконаправления
  // human_kod - код по базе HUMAN
  ...
  Select ONKNA
  find ( Str( human_kod, 7 ) )
  Do While onkna->kod == human_kod .and. !Eof()
...
Function f1_create2reestr19_( _nyear, _nmonth )
