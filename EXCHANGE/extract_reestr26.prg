...
// 11.01.26 вынуть реестр из XML-файлов и записать во временные DBF-файлы
Function extract_reestr26( mkod, mname_xml, flag_tmp1, is_all, goal_dir )

  Local oXmlDoc, oXmlNode
  local oNode1, oNode11, oNode100, oNode2, oNode3, oNode4, oNode5
  Local p_tip_reestr, lal
  Local tmpSelect
  Local arr_f, i, ii, j, j1, j2, j3, j4, isl, iisl, _ar
  Local ushifr
  local lREGNUM, lREGNUM_DOP, lCODE_SH
  local name_zip, typeFile
  Local pole, s, jkslp
  Local buf := save_maxrow(), fl := .f.
  ...
  Local _table1 := { ;
    ...
    { 'ENP',      'C', 16, 0 }, ; //
    { 'SMO',      'C',  5, 0 }, ; //
    { 'SMO_OGRN', 'C', 15, 0 }, ; // ОГРН СМО
    { 'SMO_OK',   'C',  5, 0 }, ; // ОКАТО территории страхования
    { 'SMO_NAM',  'C',100, 0 }, ; // Наименование СМО
    { 'INV',      'C',  1, 0 }, ; // Указывается группа инвалидности
    { 'MO_PR',    'C',  6, 0 }, ; //
    { 'NOVOR',    'C',  9, 0 }, ; //
...
*/
  ...
  //
  Default flag_tmp1 To .f.
  default is_all To .t.
  default goal_dir To dir_server() + dir_XML_MO() + hb_ps()
//  Private pole

  mname_xml := AllTrim( mname_xml )
  name_zip := mname_xml + szip()

  typeFile := Left( mname_xml, 2 )
  if eq_any( typeFile, 'HM', 'FM' )
    p_tip_reestr := iif( typeFile == 'HM', 1, 2 )
  else
    p_tip_reestr := iif( Left( mname_xml, 3 ) == 'HRM', 1, 2 )
  endif
    stat_msg( 'Распаковка/чтение/анализ ' + iif( eq_any( typeFile, 'HM', 'FM' ), 'счёта ', 'реестра ' ) + mname_xml )

  If ( arr_f := extract_zip_xml( goal_dir, name_zip ) ) != NIL
    fl := .t.
    dbCreate( cur_dir() + 'tmp_r_t1', _table1, , .t., 'T1' )
    dbCreate( cur_dir() + 'tmp_r_t1_1', _table1, , .t., 'T1_1' )
    dbCreate( cur_dir() + 'tmp_r_t2', _table2, , .t., 'T2' )
    dbCreate( cur_dir() + 'tmp_r_t3', _table3, , .t., 'T3' )
    dbCreate( cur_dir() + 'tmp_r_t4', _table4, , .t., 'T4' )
    dbCreate( cur_dir() + 'tmp_r_t5', _table5, , .t., 'T5' )
    dbCreate( cur_dir() + 'tmp_r_t6', _table6, , .t., 'T6' )
    dbCreate( cur_dir() + 'tmp_r_t7', _table7, , .t., 'T7' )
    dbCreate( cur_dir() + 'tmp_r_t8', _table8, , .t., 'T8' )
    dbCreate( cur_dir() + 'tmp_r_t9', _table9, , .t., 'T9' )
    dbCreate( cur_dir() + 'tmp_r_t10', _table10, , .t., 'T10' )
    dbCreate( cur_dir() + 'tmp_r_t11', _table11, , .t., 'T11' )
    dbCreate( cur_dir() + 'tmp_r_t12', _table12, , .t., 'T12' )
        ...
        Case 'ZAP' == oXmlNode:title
          If is_all
            ...
            If ( oNode1 := oXmlNode:find( 'PACIENT' ) ) != NIL
              ...
              t1->INV     := mo_read_xml_stroke( oNode1, 'INV' )
