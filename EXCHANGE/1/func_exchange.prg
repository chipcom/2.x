...
// 30.06.24
function create_node_patient( oXml, tip_reestr, fl_ver )

  local oPac, arr_fio, smr

  arr_fio := retfamimot( 2, .f. )
    // заполним сведения о пациенте для XML-документа
    oPAC := oXml:aItems[ 1 ]:add( hxmlnode():new( 'PERS' ) )
    ...
    If human_->NOVOR == 0
      ...
      Endif
      If tip_reestr == 2 // Указывается только для диспансеризации при предоставлении сведений
