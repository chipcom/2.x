...
//  mo_xml->FNAME2 := 'L' + s
  ...
  //
  // создадим новый XML-документ для пациентов
  ...
  If StrZero( _nyear, 4 ) + StrZero( _nmonth, 2 ) > '201910' // с ноября 2019 года
    fl_ver := 32
//    s := '3.2'
    mo_add_xml_stroke( oXmlNodePacient, 'VERSION', '3.2' )
  else
    fl_ver := 311
//    s := '3.11'
    mo_add_xml_stroke( oXmlNodePacient, 'VERSION', '3.11' )
  Endif
//  mo_add_xml_stroke( oXmlNodePacient, 'VERSION',s )
  ...
  Do While !Eof()
    ...
    create_node_patient( oXmlDocPacient, p_tip_reestr, fl_ver )
...
//    // заполним сведения о пациенте для XML-документа
//    oPAC := oXmlDocPacient:aItems[ 1 ]:add( hxmlnode():new( 'PERS' ) )
//    mo_add_xml_stroke( oPAC, 'ID_PAC', human_->ID_PAC )
//    If human_->NOVOR == 0
...
//  Enddo
  ...
  t2 := seconds() - t1
  alert( 'Время конвертации - ' + sectotime( t2 ) )  
